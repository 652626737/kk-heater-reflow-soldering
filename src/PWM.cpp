...
void PWM::begin()
{
    ...
    {
        if (temp.now_temp > curve_temp_buf[0])
            return;
        backflow_working_state = 1;
        backflow_temp_tmp = temp.now_temp;
        percent = 0;
    }
    pinMode(PWM_IO, OUTPUT);
    digitalWrite(PWM_IO, LOW);
...
void PWM::temp_set()
{
    ...
    if (temp_mode == Co_Temp)
    {
        ...
        if (temp.now_temp >= temp_buf && temp_reached_flg == false)
        {
    ...
    ek[2] = ek[1];
    ek[1] = ek[0];
    ek[0] = need_set_temp - temp.now_temp;

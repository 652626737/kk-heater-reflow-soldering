...
void s1_tic()
{
  ...
  if(pwm.backflow_working_state == 1)
  {
    ...
    if(tmp == 0)
      tmp = 1;
    pwm.percent = (temp.now_temp - 38) * 10 / tmp;
    ...
    else
    {
      ...
      if(temp.now_temp >= pwm.backflow_temp_tmp)
      {
  ...
  else if(pwm.backflow_working_state == 3)
  {
    ...
    if(tmp == 0)
      tmp = 1;
    pwm.percent = 50 + (temp.now_temp - curve_temp_buf[0]) * 10 / tmp;
    if(temp.now_temp > 215)
    {

...
const char menu3_option1_2[] = {"KK-Heater"};

const char menu5_option0_0[] = {"这么简单自己摸索"};

const char menu6_option0_0[] = {"ESP32智能加热台"};
const char menu6_option0_1[] = {"Version:V2.0"};
...
void UI::page1_key(ec11_task_result_type ec_type, int16_t ec_value) //主界面按键对应功能
{
    ...
    else
    {
        if (pwm.temp_mode)
        {
            ...
            if (pwm.temp_buf > temp.hotbed_max_temp)
                pwm.temp_buf = temp.hotbed_max_temp;
...
void UI::page3_key(ec11_task_result_type ec_type, int16_t ec_value) //界面3按键对应功能
{
        ...
        if (ec_value == sw_click) //单击
        {
            ...
            if (page2_menu_num == error_temp_fix_num)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (pwm.power == 0 && temp.now_temp < 150)
                    {
    ...
    else
    {
        ...
        case backflow_menu_num: //回流参数
            if (!page3_switch_flg)
            {
                ...
                case 2:
                    ...
                    else if (curve_temp_buf[2] > temp.hotbed_max_temp)
                        curve_temp_buf[2] = temp.hotbed_max_temp;
                    break;
        ...
        case error_temp_fix_num: //温度校准
            ...
            case 1:
            {
                temp.hotbed_max_temp += ec_value;
                if (temp.hotbed_max_temp < 240)
                    temp.hotbed_max_temp = 240;
                else if (temp.hotbed_max_temp > 270)
                    temp.hotbed_max_temp = 270;
            }
            break;
            case 2:
                if (ec_value < 0)
...
void UI::blinker_config()
{
    ...
    wifima.startConfigPortal("KK-Heater");
...
void UI::show_page(short x, short y, uint8_t page)
{
    ...
    case 3:
        ...
        case error_temp_fix_num: //温度校准
            ...
            oled.num(72, y, temp.hotbed_max_temp, 3, 16, LEFT, 1);
            oled.num(72, y + 16, temp.adc_max_temp, 3, 16, LEFT, 1);
...
void UI::error_temp_fix_page_move()
{
    ...
    else
    {
        ...
        for (uint8_t x = 0; x < 33; x++)
        {
            ...
            oled.num(64, tmp + 48, temp.now_temp, 3, 16, LEFT, 1);
            oled.refresh();
            if (error_temp_fix_page_buf == 2)
                tmp++;
            else
        ...
        if (error_temp_fix_page_buf == 3)
        {
            ...
            {
                if (temp.adc_max_temp_auto())
                {
                    digitalWrite(PWM_IO, LOW);
                    break;
                }
                oled.choose_clr(64, 2, 24, 2);
                oled.num(64, 16, temp.now_temp, 3, 16, LEFT, 1);
                oled.refresh();
                delay(1000);
            }
...
void UI::show_temp(int8_t x, int8_t y, int8_t xx, int8_t yy)
{
    ...
    oled.num(128, 0, (int16_t)temp.environmental_temp, 3, 16, RIGHT, 1);

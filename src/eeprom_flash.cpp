...
void eeprom_flash::data_init()
{
    ...
    ui.oled_light = 127;
    temp.adc_max_temp = 255;
    temp.hotbed_max_temp = 255;
...
void eeprom_flash::read_all_data()
{
    ...
    else
    {
        ...
        temp.hotbed_max_temp = ((uint16_t)EEPROM.read(hotbed_max_temp_high_add) << 8) | EEPROM.read(hotbed_max_temp_low_add);
        if (temp.hotbed_max_temp > 270 || temp.hotbed_max_temp < 240)
        {
            temp.hotbed_max_temp = 255;
            EEPROM.write(hotbed_max_temp_low_add, temp.hotbed_max_temp & 0xff);
            EEPROM.write(hotbed_max_temp_high_add, temp.hotbed_max_temp >> 8);
        }
        temp.adc_max_temp = ((uint16_t)EEPROM.read(adc_max_temp_high_add) << 8) | EEPROM.read(adc_max_temp_low_add);
        if (temp.adc_max_temp > 400 || temp.adc_max_temp < 190)
        {
            temp.adc_max_temp = 255;
